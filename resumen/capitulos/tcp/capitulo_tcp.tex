...
Luego de cada segmento validado, la \texttt{cwnd} se duplica. Cuando \texttt{cwnd >= ssthresh}, TCP detiene el crecimiento de \texttt{cwnd}.

Cuando \texttt{cwnd > ssthresh}, por cada ACK \texttt{cwnd = cwnd + 1}, es decir, se incrementa de forma lineal. 
...
Luego de esa retransmisión, se pasa a \textbf{Fast Recovery} reduciendo \texttt{cwnd = ssthresh = cwnd\_anterior / 2}. Si hay un \textbf{timeout}, la \texttt{cwnd} se reinicia a 1.